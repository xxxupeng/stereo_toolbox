...
def auto_mask(left_image, right_image, disp, denorm=False, stationary=True, occlusion=False):
    if denorm:
    ...
    mask = torch.ones_like(reproj_error, dtype=torch.bool)
    ...
    if occlusion:
        occlusion_threshold = reproj_error.mean(dim=(-1,-2), keepdim=True) * 2
