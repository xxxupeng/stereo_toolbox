...
class FoundationStereo(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    def __init__(self, args=None):
        ...
        self.cnet = ContextNetDino(self.args, output_dim=[self.args.hidden_dims, context_dims], downsample=self.args.n_downsample)
        ...
        self.context_zqr_convs = nn.ModuleList([nn.Conv2d(context_dims[i], self.args.hidden_dims[i]*3, kernel_size=3, padding=3//2) for i in range(self.args.n_gru_layers)])

        self.feature = Feature(self.args)
    ...
    def forward(self, image1, image2, iters=None, flow_init=None, test_mode=None, low_memory=None, init_disp=None):
