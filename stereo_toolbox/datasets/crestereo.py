...
class CREStereo_Dataset(Dataset):
    """
    Inputs:
    - split: 'train'
    ...
    """
    def __init__(self, split: str, training: bool, root_dir='/data/xp/CREStereo/', imagenet_norm=True):
        assert split in ['train'], "Invalid split name"
        self.split = split
        self.training = training

        dataset_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    f'datasets_lists/crestereo/{self.split}.txt')
    ...
    def load_disp(self, filename: str):
        ...
        disp = np.array(disp, dtype=np.float32) / 32.
        return disp
