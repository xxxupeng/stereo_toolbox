...
class MonoTrap_Dataset(Dataset):
    """
    Inputs:
    - split: 'val'
    ...
    """
    def __init__(self, split: str, training: bool, root_dir='/data/xp/MonoTrap/', imagenet_norm=True):
        assert split in ['val'], "Invalid split name"
        self.split = split
        self.training = training

        dataset_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    f'datasets_lists/monotrap/{self.split}.txt')
