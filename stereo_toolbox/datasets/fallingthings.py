...
class FallingThings_Dataset(Dataset):
    """
    Inputs:
    - split: 'train'
    ...
    """
    def __init__(self, split: str, training: bool, root_dir='/data1/xp/FallingThings/'):
        assert split in ['train'], "Invalid split name"
        self.split = split
        self.training = training

        dataset_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    f'datasets_lists/fallingthings/{self.split}.txt')
    ...
    def load_disp(self, filename: str):
        ...
        disp = (768.1605834960938 * 6.0 * 100) / np.array(disp, dtype=np.float32)
        return disp
