...
class InStereo2k_Dataset(Dataset):
    """
    Inputs:
    - split: 'train', 'test'
    ...
    """
    def __init__(self, split: str, training: bool, root_dir='/data/xp/InStereo2k/', imagenet_norm=True):
        assert split in ['train', 'test'], "Invalid split name"
        self.split = split
        self.training = training

        dataset_file = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    f'datasets_lists/instereo2k/{self.split}.txt')
    ...
    def load_disp(self, filename: str):
        ...
        disp = np.array(disp, dtype=np.float32) / 1024.
        return disp
